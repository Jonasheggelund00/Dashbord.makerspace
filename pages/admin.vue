<template>
  <div :class="isDark ? 'min-h-screen bg-gradient-to-b from-gray-900 to-gray-800' : 'min-h-screen bg-gradient-to-b from-gray-50 to-white'">
    <Header />
    <div class="container mx-auto max-w-7xl p-6 mt-8">
      ...
      <!-- Dashboard Tab -->
      <div v-show="activeTab === 'dashboard'" :class="isDark ? 'space-y-6' : 'space-y-6'">
        <!-- Statistikk kort -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
            <div class="flex items-center justify-between">
              <div>
                <p :class="isDark ? 'text-gray-400 text-sm font-medium' : 'text-gray-600 text-sm font-medium'">Totalt printere</p>
                <p :class="isDark ? 'text-3xl font-bold text-white mt-2' : 'text-3xl font-bold text-gray-900 mt-2'">{{ printers.length }}</p>
              </div>
              <div :class="isDark ? 'bg-blue-900/30 p-3 rounded-lg' : 'bg-blue-100 p-3 rounded-lg'">
          ...
          <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
            <div class="flex items-center justify-between">
              <div>
                <p :class="isDark ? 'text-gray-400 text-sm font-medium' : 'text-gray-600 text-sm font-medium'">Aktive prints</p>
                <p :class="isDark ? 'text-3xl font-bold text-white mt-2' : 'text-3xl font-bold text-gray-900 mt-2'">{{ activePrintsCount }}</p>
              </div>
              <div :class="isDark ? 'bg-green-900/30 p-3 rounded-lg' : 'bg-green-100 p-3 rounded-lg'">
          ...
          <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
            <div class="flex items-center justify-between">
              <div>
                <p :class="isDark ? 'text-gray-400 text-sm font-medium' : 'text-gray-600 text-sm font-medium'">Totale logger</p>
                <p :class="isDark ? 'text-3xl font-bold text-white mt-2' : 'text-3xl font-bold text-gray-900 mt-2'">{{ totalLogsCount }}</p>
              </div>
              <div :class="isDark ? 'bg-purple-900/30 p-3 rounded-lg' : 'bg-purple-100 p-3 rounded-lg'">
          ...
          <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
            <div class="flex items-center justify-between">
              <div>
                <p :class="isDark ? 'text-gray-400 text-sm font-medium' : 'text-gray-600 text-sm font-medium'">Printer feil</p>
                <p :class="isDark ? 'text-3xl font-bold text-white mt-2' : 'text-3xl font-bold text-gray-900 mt-2'">{{ printerErrorsCount }}</p>
              </div>
              <div :class="isDark ? 'bg-red-900/30 p-3 rounded-lg' : 'bg-red-100 p-3 rounded-lg'">
        ...
        <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
          <h2 :class="isDark ? 'text-2xl font-bold mb-4 text-white' : 'text-2xl font-bold mb-4 text-gray-800'">Aktivitet siste 7 dager</h2>
          <div class="space-y-3">
            <div v-for="day in last7DaysActivity" :key="day.date" class="flex items-center justify-between">
              <span :class="isDark ? 'text-gray-300' : 'text-gray-700'">{{ day.date }}</span>
              <div class="flex items-center gap-4">
                <span :class="isDark ? 'text-green-400' : 'text-green-600'">{{ day.starts }} starts</span>
                <span :class="isDark ? 'text-blue-400' : 'text-blue-600'">{{ day.ends }} ferdig</span>
                <span :class="isDark ? 'text-red-400' : 'text-red-600'">{{ day.errors }} feil</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
          <h2 :class="isDark ? 'text-2xl font-bold mb-4 text-white' : 'text-2xl font-bold mb-4 text-gray-800'">Mest brukte printere (siste 30 dager)</h2>
          <div class="space-y-3">
            <div v-for="printer in topPrinters" :key="printer.name" class="flex items-center justify-between">
              <span :class="isDark ? 'text-gray-300 font-medium' : 'text-gray-700 font-medium'">{{ printer.name }}</span>
              <div class="flex items-center gap-3">
                <div class="w-48 bg-gray-200 rounded-full h-2.5">
                  <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: printer.percentage + '%' }"></div>
                </div>
                <span :class="isDark ? 'text-gray-400 text-sm' : 'text-gray-600 text-sm'">{{ printer.count }} prints</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Printers Tab -->
      <div v-show="activeTab === 'printers'" :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
        <h1 :class="isDark ? 'text-3xl font-bold mb-6 text-white' : 'text-3xl font-bold mb-6 text-gray-800'">Administrer printere</h1>
        
        <form @submit.prevent="addPrinter" class="mb-8 flex flex-col gap-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label :class="isDark ? 'block mb-1 font-semibold text-gray-300' : 'block mb-1 font-semibold text-gray-700'">IP-adresse</label>
            ...
            <div>
              <label :class="isDark ? 'block mb-1 font-semibold text-gray-300' : 'block mb-1 font-semibold text-gray-700'">API Key</label>
        ...
        <h2 :class="isDark ? 'text-2xl font-bold mb-4 text-white' : 'text-2xl font-bold mb-4 text-gray-800'">Eksisterende printere</h2>
        ...
        <div v-if="printers.length === 0" :class="isDark ? 'text-gray-400 text-center py-8' : 'text-gray-500 text-center py-8'">Ingen printere lagt til.</div>
        <div v-else-if="filteredAndSortedPrinters.length === 0" :class="isDark ? 'text-gray-400 text-center py-8' : 'text-gray-500 text-center py-8'">
          Ingen printere matcher filteret.
        </div>
        <div v-else class="overflow-x-auto">
          <table :class="isDark ? 'w-full bg-gray-700 rounded-lg overflow-hidden' : 'w-full bg-gray-50 rounded-lg overflow-hidden'">
            <thead :class="isDark ? 'bg-gray-600' : 'bg-gray-200'">
              <tr>
                <th :class="isDark ? 'py-3 px-4 text-left font-semibold text-gray-200' : 'py-3 px-4 text-left font-semibold'">
                ...
                <th :class="isDark ? 'py-3 px-4 text-left font-semibold text-gray-200' : 'py-3 px-4 text-left font-semibold'">API Key</th>
                <th :class="isDark ? 'py-3 px-4 text-left font-semibold text-gray-200' : 'py-3 px-4 text-left font-semibold'"></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="printer in filteredAndSortedPrinters" :key="printer.ip" :class="isDark ? 'border-b border-gray-600 group hover:bg-gray-600 transition' : 'border-b group hover:bg-red-50 transition'">
                <td class="py-3 px-4">
                  <template v-if="editNameIp === printer.ip">
                    <input v-model="editNameValue" :class="isDark ? 'border border-gray-500 bg-gray-800 text-white rounded px-2 py-1 w-32' : 'border rounded px-2 py-1 w-32'"
                  ...
                  <template v-else>
                    <span :class="isDark ? 'font-medium text-white' : 'font-medium'">{{ printer.displayName || '-' }}</span>
                    <button @click="startEditName(printer)" :class="isDark ? 'ml-2 text-gray-400 hover:text-blue-400' : 'ml-2 text-gray-400 hover:text-blue-600'">✎</button>
                  </template>
                </td>
                <td :class="isDark ? 'py-3 px-4 text-gray-300' : 'py-3 px-4'">{{ printer.hostname || '-' }}</td>
                <td :class="isDark ? 'py-3 px-4 font-mono text-sm text-gray-300' : 'py-3 px-4 font-mono text-sm'">{{ printer.ip }}</td>
                <td :class="isDark ? 'py-3 px-4 font-mono text-xs text-gray-400' : 'py-3 px-4 font-mono text-xs text-gray-600'">{{ printer.apiKey }}</td>
        ...
        <div v-if="showConfirm" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div :class="isDark ? 'bg-gray-800 border border-gray-700 rounded-lg shadow-lg p-8 max-w-sm w-full' : 'bg-white rounded-lg shadow-lg p-8 max-w-sm w-full'">
            <h3 :class="isDark ? 'text-lg font-bold mb-4 text-white' : 'text-lg font-bold mb-4'">Er du sikker på at du vil slette denne printeren?</h3>
...
>
  ...
  <button
    ...
    aria-label="Lukk informasjonsboksen"
    title="Lukk"
  >
    <!-- X ikon -->
  ...
  <p :class="isDark ? 'text-blue-200 text-sm' : 'text-blue-800 text-sm'">
    ...
    Systemet logger automatisk når enheter slås på/av og når prints starter. Dette er tilgjengelig kun for admin-brukere.
  </p>
</div>
        ...
        <div v-if="logs.length > 0">
          <!-- Tabell for logger -->
          <div :class="isDark ? 'bg-gray-700 border border-gray-600 rounded-lg overflow-hidden' : 'bg-white border border-gray-200 rounded-lg overflow-hidden'">
            <table class="w-full">
              <thead :class="isDark ? 'bg-gray-600 border-b border-gray-500' : 'bg-gray-100 border-b border-gray-200'">
                <tr>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Status</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Enhet</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Dato</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Tid</th>
                  <th :class="isDark ? 'px-4 py-3 text-center text-xs font-semibold text-gray-200 uppercase tracking-wider w-16' : 'px-4 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wider w-16'"></th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200">
                <tr v-for="log in paginatedLogs" :key="log.id" class="hover:bg-gray-50 transition-colors">
                  <td class="px-4 py-3">
                    <span :class="['px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap inline-block', getLogTypeClass(log.type)]">
          ...
          <div v-if="hasMoreLogs" class="flex justify-between items-center mt-6 pt-4 border-t border-gray-200">
            <div class="text-sm text-gray-600">
              Viser {{ ((currentPage - 1) * logsPerPage) + 1 }}-{{ Math.min(currentPage * logsPerPage, logs.length) }} av {{ logs.length }} logger
            </div>
            <div class="flex gap-2">
              <button 
                @click="currentPage--"
      ...
      <div v-show="activeTab === 'users'" :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
        ...
        <!-- Info box -->
        <div :class="isDark ? 'bg-blue-900/30 border border-blue-700 rounded-lg p-4 mb-6' : 'bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6'">
          <p :class="isDark ? 'text-blue-200 text-sm' : 'text-blue-800 text-sm'">
            <strong>Om brukeraktivitet:</strong> Her kan du se hvem som har logget inn i admin-panelet, når de logget inn og fra hvilken IP-adresse.
          </p>
        </div>
        ...
        <div v-if="userActivityLogs.length > 0">
          <div :class="isDark ? 'bg-gray-700 border border-gray-600 rounded-lg overflow-hidden' : 'bg-white border border-gray-200 rounded-lg overflow-hidden'">
            <table class="w-full">
              <thead :class="isDark ? 'bg-gray-600 border-b border-gray-500' : 'bg-gray-100 border-b border-gray-200'">
                <tr>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Bruker</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Handling</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">IP-adresse</th>
                  <th :class="isDark ? 'px-4 py-3 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider' : 'px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider'">Dato & Tid</th>
                </tr>
              </thead>
              <tbody :class="isDark ? 'divide-y divide-gray-600' : 'divide-y divide-gray-200'">
                <tr v-for="activity in userActivityLogs" :key="activity.id" :class="isDark ? 'hover:bg-gray-600 transition-colors' : 'hover:bg-gray-50 transition-colors'">
                  <td :class="isDark ? 'px-4 py-3 font-medium text-white' : 'px-4 py-3 font-medium text-gray-800'">{{ activity.username }}</td>
                  <td class="px-4 py-3">
                    <span :class="[
                      ...
                      <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <circle v-else cx="10" cy="10" r="6" fill="none" stroke="currentColor" stroke-width="2"></circle>
                      </svg>
                      {{ activity.action === 'login' ? 'Logget inn' : 'Logget ut' }}
                    </span>
                  </td>
                  <td :class="isDark ? 'px-4 py-3 text-sm text-gray-300 font-mono' : 'px-4 py-3 text-sm text-gray-600 font-mono'">{{ activity.ip || 'N/A' }}</td>
                  <td :class="isDark ? 'px-4 py-3 text-sm text-gray-300' : 'px-4 py-3 text-sm text-gray-600'">{{ formatDateTime(activity.timestamp) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div v-show="activeTab === 'accounts'" :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
      ...
      <!-- Settings Tab -->
      <div v-show="activeTab === 'settings'" :class="isDark ? 'bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-700' : 'bg-white rounded-xl shadow-lg p-6'">
        <h1 :class="isDark ? 'text-3xl font-bold mb-6 text-white' : 'text-3xl font-bold mb-6 text-gray-800'">Systeminnstillinger</h1>
        
        <div class="space-y-6">
          <!-- Home Assistant Settings -->
          <div :class="isDark ? 'border border-gray-700 rounded-lg p-6' : 'border border-gray-200 rounded-lg p-6'">
            <h2 :class="isDark ? 'text-xl font-bold mb-4 text-white' : 'text-xl font-bold mb-4 text-gray-800'">Home Assistant</h2>
            <div class="space-y-4">
              <div>
                <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-2' : 'block text-sm font-medium text-gray-700 mb-2'">Base URL</label>
              ...
              <form @submit.prevent="testHAConnection" class="space-y-4">
                <div>
                  <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-2' : 'block text-sm font-medium text-gray-700 mb-2'">Access Token</label>
              ...
              <div v-if="haTestResult" :class="haTestResult.success ? 'text-green-600' : 'text-red-600'" class="text-sm font-medium">
          ...
          <!-- Polling Settings -->
          <div :class="isDark ? 'border border-gray-700 rounded-lg p-6' : 'border border-gray-200 rounded-lg p-6'">
            <h2 :class="isDark ? 'text-xl font-bold mb-4 text-white' : 'text-xl font-bold mb-4 text-gray-800'">Oppdateringsintervaller</h2>
            <div class="space-y-4">
              <div>
                <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-2' : 'block text-sm font-medium text-gray-700 mb-2'">
                  Printer polling (sekunder)
                </label>
                <input v-model.number="pollingSettings.printerInterval" type="number" min="1" max="300"
                  :class="isDark ? 'w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white' : 'w-full px-3 py-2 border border-gray-300 rounded-md'" />
                <p :class="isDark ? 'text-xs text-gray-400 mt-1' : 'text-xs text-gray-500 mt-1'">Hvor ofte alle brukere skal se oppdatert printer-status (1-300 sekunder). Standard: 3 sekunder</p>
              </div>
              <div>
                <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-2' : 'block text-sm font-medium text-gray-700 mb-2'">
                ...
                <input v-model.number="pollingSettings.sensorInterval" type="number" min="1" max="300"
                  :class="isDark ? 'w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white' : 'w-full px-3 py-2 border border-gray-300 rounded-md'" />
                <p :class="isDark ? 'text-xs text-gray-400 mt-1' : 'text-xs text-gray-500 mt-1'">Hvor ofte systemet skal sjekke sensor-status (1-300 sekunder). Standard: 10 sekunder</p>
              </div>
            </div>
          </div>
          ...
          <!-- Log Retention -->
          <div :class="isDark ? 'border border-gray-700 rounded-lg p-6' : 'border border-gray-200 rounded-lg p-6'">
            <h2 :class="isDark ? 'text-xl font-bold mb-4 text-white' : 'text-xl font-bold mb-4 text-gray-800'">Logger</h2>
            <div class="space-y-4">
              <div>
                <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-2' : 'block text-sm font-medium text-gray-700 mb-2'">
                ...
                <input v-model.number="logSettings.retentionDays" type="number" min="1" max="365"
                  :class="isDark ? 'w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white' : 'w-full px-3 py-2 border border-gray-300 rounded-md'" />
                <p :class="isDark ? 'text-xs text-gray-400 mt-1' : 'text-xs text-gray-500 mt-1'">Logger eldre enn dette slettes automatisk</p>
              </div>
            </div>
          </div>
          ...
          <div :class="isDark ? 'border border-gray-700 rounded-lg p-6' : 'border border-gray-200 rounded-lg p-6'">
            <h2 :class="isDark ? 'text-xl font-bold mb-1 text-white' : 'text-xl font-bold mb-1 text-gray-800'">LED-strip terskelverdier (termalkamera)</h2>
            <p :class="isDark ? 'text-sm text-gray-400 mb-4' : 'text-sm text-gray-500 mb-4'">Bestemmer hvilke temperaturer som endrer fargen på LED-stripen basert på termalkameraet.</p>
            <div class="space-y-4">
              <div class="flex items-center gap-4">
                <div class="w-4 h-4 rounded-full bg-green-500 flex-shrink-0"></div>
                <div class="flex-1">
                  <label :class="isDark ? 'block text-sm font-medium text-gray-300 mb-1' : 'block text-sm font-medium text-gray-700 mb-1'">
          ...
          <button @click="saveSettings" 
            ...
            <svg v-if="savingSettings" class="w-5 h-5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
            </svg>
          ...
          <div v-if="settingsSaved" class="text-green-600 text-center font-medium flex items-center justify-center gap-2">
  ...
  <div v-if="showDeleteLogConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      ...
      <div class="flex gap-3 justify-end">
        <button 
          @click="showDeleteLogConfirm = false; logToDelete = null"
  ...
  <div v-if="editUserModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      ...
      <form @submit.prevent="saveEditUser" class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Nytt passord <span class="text-gray-400 font-normal">(la stå tomt for å beholde)</span></label>
  ...
  <div v-if="showDeleteAdminUserConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <h3 class="text-lg font-semibold mb-4 text-red-600">Slett bruker</h3>
      <p class="text-gray-600 mb-6">Er du sikker på at du vil slette brukeren <strong>{{ adminUserToDelete?.username }}</strong>?</p>
      <div class="flex gap-3 justify-end">
        <button @click="showDeleteAdminUserConfirm = false; adminUserToDelete = null"
          class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
          Avbryt
        </button>
        <button @click="deleteAdminUser"
          class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
          Slett
        </button>
      </div>
  ...
  <div v-if="showDeleteUserActivityConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      ...
      <div class="flex gap-3 justify-end">
        <button
          @click="showDeleteUserActivityConfirm = false"
          class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50"
        >
          Avbryt
        </button>
        <button
          @click="deleteAllUserActivity"
          class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700"
        >
          Slett alt
        </button>
